...
PHASE 2 — REFINE (every reply after the engineer responds): Ask ONE focused confirming or refining question per turn, anchored to your current draft. Only ask about points the spec left open or that the engineer flagged — do NOT re-interrogate fields the spec already answered. Re-emit the updated JSON block whenever an answer changes the machine.`
    : `Interview the engineer to define THIS MODULE'S OWN states and the transitions between them. One question per turn.`;
...
2. The transitions. For each: from_state_id, to_state_id, a trigger (either {kind:"command", expr: <permissive on an operator/HMI tag>} for manual, or {kind:"completion"} when a sequential state finishes), and an optional permissive guard (array of {tag, operator, value}); a guard may reference OTHER modules' tags for inter-module interlocks.
   ...
   - To express "ANY OF these faults → Faulted" (the universal fault fan-in), emit ONE separate transition per fault tag: same from_state_id/to_state_id/guard, each with its own single-condition trigger and a unique transition_id (e.g. "<from>_to_faulted__cm1_fault", "<from>_to_faulted__cm2_therm"). The parallel transitions together ARE the OR.
...
\`\`\`json
{
  ...
  "transitions": [
    ...
    {
      "transition_id": "driving_fwd_to_faulted__vsd1_trip",
    ...
    {
      "transition_id": "driving_fwd_to_faulted__cm1_therm",
      "from_state_id": "driving_fwd",
      "to_state_id": "faulted",
...
(The two \`*_to_faulted__*\` transitions above show the fault fan-in: ONE single-condition transition per fault tag — together they mean "VSD1_CB_Trip OR CM1_Therm → Faulted". Never collapse them into an array expr.)
...
export function buildEmStateMachineOpeningMessage(
  ...
  if (sourceSections.length > 0) {
    return `Generate the opening message for the state-machine interview of equipment module "${equipmentModule.equipment_module_name}". The customer specification context for this module is in your system prompt. Execute PHASE 1 (GROUND): read it, then PROPOSE your draft state machine — the states you inferred (with the single safe state marked) and the transitions between them — citing the spec, flagging any gaps as assumptions, and ending with the JSON block plus the specific points you need the engineer to confirm. Lead with your proposal; do NOT ask a cold open-ended question.`;
  }
