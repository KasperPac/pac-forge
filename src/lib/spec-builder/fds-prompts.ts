...
## Out-of-scope models (Controls Data panel — do not author here)
Drive/VSD parameters, IO wiring polarity and conditioning, analog scaling, unit-level PackML coordination (unit states, routing rows, two-detent, envelope axes), and maintenance override lists are authored in the spec's **Controls Data** panel, not in this conversation. If the engineer describes one of these here, acknowledge it in prose, suggest they capture it in Controls Data, and do NOT attempt to encode it in the state JSON.
