...
vi.mock("@/hooks/use-fb-templates", () => ({
  useFbTemplates: () => ({
    data: [
      {
        id: "tpl-1", name: "Motor_Std", device_category: "motor", plc_brand: "SIEMENS_TIA",
        ...
        source: "library", library_name: "LGF", is_enabled: true, is_equipment_module: false,
        documentation: null, hmi_faceplate_type: null, created_by: null, updated_at: "", created_at: "",
        interface_contract: {
          block_name: "CM_Motor",
          pins: [
            { name: "fb_run", scl_type: "Bool", direction: "input", role: "sensor_in", default_binding: "io_input", exposed: true, description: "" },
            { name: "act_run", scl_type: "Bool", direction: "output", role: "actuator_out", default_binding: "io_output", exposed: true, description: "" },
          ],
          states: [], reviewed: true, generated_at: "",
        },
      },
    ],
  }),
...
describe("ControlsDataPanel — FB assignments (G6-4)", () => {
  it("authors an assignment with a pin binding and saves it into engineering.fb_assignments", async () => {
    const s = fixtureSpec();
    s.confirmed_units![0].equipment_modules = [
      {
        equipment_module_id: "em-a", equipment_module_name: "Drive EM", description: "",
        control_modules: [
          {
            control_module_id: "cm-1", control_module_name: "M01", control_module_class: "motor",
            description: "", is_safety: false,
            io_signals: [
              { tag: "M01_Run", signal_type: "DO", io_address: "Q0.0", description: "" },
              { tag: "M01_FB", signal_type: "DI", io_address: "I0.0", description: "" },
            ],
          },
        ],
      },
    ...
    fireEvent.click(screen.getByText("pin binding"));
    fireEvent.click(screen.getByText("Save controls data"));

    await waitFor(() => expect(writeSpecContract).toHaveBeenCalled());
    const patch = vi.mocked(writeSpecContract).mock.calls[0][1];
    const a = patch.engineering?.fb_assignments[0];
    // the EM has no EM-level template, so the first compatible pair is the CM
    expect(a).toMatchObject({
      target_kind: "control_module",
      target_id: "cm-1",
      template_id: "tpl-1",
    });
    expect(a?.pin_bindings[0]).toEqual({ pin: "fb_run", tag: "M01_Run" });
  });
});
