...
import { Loader2, CheckCircle2, AlertCircle, ArrowRight, Database, Cpu, Tag, Monitor, FolderTree } from "lucide-react";
...
export function AuditExtract({ session, onSessionUpdate }: AuditExtractProps) {
  ...
  const extractMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      updateCategory("HMI Screens", { status: "done", count: 0 }); // HMI extraction is Phase 2+

      // Persist extracted data to Supabase
      try {
        await persistExtraction(session.id, data);
...
async function persistExtraction(auditProjectId: string, data: ExtractProjectResponse) {
