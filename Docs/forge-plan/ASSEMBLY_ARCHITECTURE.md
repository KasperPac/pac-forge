# Assembly FB Architecture Plan

## Machine Hierarchy

```
System (the full machine / production line)
  └── Subsystem (functional station)
        └── Assembly (coordinated group of devices)
...
├── Subsystem: Processing Station
│   └── Assembly: PRESS01 Stamping Press
...
3. **Assemblies get their own FBs** — coordination logic, fault handling, HMI UDT
4. **Process sequences command assemblies** — not individual devices
5. **Subsystems are organisational** — each typically has its own process sequence(s)
...
### Assembly FB Interface Example (Lift Table)
...
### Process Sequence Using Assembly

```
Step 0:  PB_UP pressed → set ProcessCommands.lft01CmdRaise = TRUE
Step 10: Wait ProcessState.lft01Busy = TRUE (assembly acknowledged)
Step 20: Wait ProcessState.lft01AtUpper = TRUE (assembly done)
...
**Files:**
- `src/types/forge.ts` — Add `SpecAnalysisAssembly`, update `SpecAnalysis` with hierarchy
- `src/types/forge-matrix.ts` — Add assembly linkage to matrix types
...
```typescript
interface SpecAnalysisAssembly {
  id: string;
  name: string;
  tag: string;
  assembly_type: string;  // "LiftTable", "Conveyor", "StampingPress"
  description: string;
  subsystem: string;
...
interface SpecAnalysis {
  ...
  subsystems: Array<{ name: string; description: string }>;  // existing
  assemblies: SpecAnalysisAssembly[];  // NEW
...
**Hardware & IO (Step 4):**
- Show assemblies as grouping headers, devices as children
- FB matching only applies to devices
- Assembly FB is always AI-generated (no library matching)

**Device FBs (Step 5):**
- Generate device FBs as today (for leaf devices only)

**NEW: Assembly FBs (Step 5.5 or merged into Step 5):**
- New wizard step or sub-step
- For each assembly: generate Assembly FB + config UDT + HMI UDT + instance DB
- Prompt includes: assembly description, list of constituent devices + their FB interfaces
- Output: assembly FB with state machine, fault logic, device coordination

**Matrix Review (Step 6):**
- Process sequences reference assemblies, not devices
- Device linkage: assembly FB wiring to ProcessCommands/ProcessState
- Wiring map shows: assembly → device chain

**Device Code (Step 7):**
- Device call FCs as today
- NEW: Assembly call FCs — call assembly FBs with wiring from matrix

**Process Code (Step 9):**
- Sequences command assemblies: `lft01CmdRaise`, `lft01AtUpper`, etc.
- Much simpler than today's device-level orchestration

### Phase 3: Assembly FB Generator

**New files:**
- `src/lib/forge-assembly-prompts.ts` — Assembly FB generation prompt builder
- `src/hooks/use-forge-assembly-generate.ts` — Assembly FB generation hook
- `src/types/forge-assembly.ts` — Assembly-specific types (if needed)
...
**Prompt outputs:**
- Assembly FB (SCL)
- Assembly config UDT (typeLiftTableConfig etc.)
- Assembly HMI UDT (udtHMI_LiftTable etc.)
- Assembly instance DB

### Phase 4: HMI Integration

- HMI faceplates per assembly (not per device)
- Assembly HMI UDT on instance DB
- Screen generation references assemblies

## What Stays the Same

- Device FBs — still generated from library templates or AI
- IO linking — still maps physical IO to/from DBs
- Pattern learning — still works on device and assembly FBs
...
| Aspect | Before | After |
|--------|--------|-------|
| Device list | Flat, includes assemblies | Hierarchical, devices only |
| LFT01 | Device with fbPulser FB | Assembly with AI-generated FB |
| Process sequences | Command individual devices | Command assemblies |
| Fault handling | In process sequences | In assembly FBs |
| HMI | Per device | Per assembly |
...
- Assembly FBs are always project-specific (AI generated, not library)

## Open Questions

1. Should assembly FBs call device FBs directly (multi-instance) or read from ProcessState?
   - Direct call: cleaner, single scan, but means device FBs called from assembly FC not device call FC
...
2. Assembly FB library — should we build a library of common assembly patterns?
...
3. Nested assemblies — can an assembly contain other assemblies?
   - e.g. "Packaging Station" contains "Conveyor" + "Wrapper" + "Labeller"
   - **Recommendation:** Keep flat for now (assembly contains only devices), add nesting later if needed
